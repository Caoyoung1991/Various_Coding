...
//  information.
//#include <tr1/functional> //这边到时候要改回来
...
private:
	...
	//     aludra.  You will need the following version on aludra.
		...
		return std::hash<string>()(word) % hashSize; //到时候要改回来
	}
	...
	//新添加
	ListType * data; //链表的首地址
	unsigned int numOfEntry;
	//unsigned int nonEtyBuckets;
	int * track;
	//unsigned int countBuckets(ListType *) const;
	unsigned int maxChainLength(int []) const;
	unsigned int countBuckets(int []) const;
};
